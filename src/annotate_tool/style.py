...
# 塗り無し(輪郭のみ)モードの線幅。半透明の塗りは下の画素を着色してしまい、
# 境界が合っているかの判定材料そのものを壊す。塗りを消したときは代わりに線を
# 太くして、画像には一切手を触れずに輪郭だけで見比べられるようにする。
CONTOUR_PEN_WIDTH = 2.5
CONTOUR_SELECTED_PEN_WIDTH = 4.5

# 輪郭の縁取り(ハロー)。写真の上ではどんな背景色が線に接するか事前に決められず、
# 単色の線ではコントラストを保証できない。暗い縁を本線の外側に敷くことで背景によらず
# 見えるようにする(航空写真上の地図ラベルと同じ手)。
HALO_COLOR = QColor(0, 0, 0, 170)
HALO_EXTRA_WIDTH = 2.0  # 本線幅への上乗せ(外側へ this/2 だけ張り出す)
# boundingRect へ足す余白(シーン座標)。ペンはコスメティック(画面ピクセル幅)
# なので、ズームアウト時はシーン座標での張り出しが 1/scale 倍に膨らむ。想定する
# 最小倍率(~0.2)でも線が矩形からはみ出さないだけの余白を固定で取る。
HALO_BOUNDS_MARGIN = 14.0

# --- オーバーレイのレイヤキャッシュ ------------------------------------------
# 数百インスタンスのポリゴンを毎フレーム描くとパンが重いので、非選択の全インスタンス
# を1枚のラスタへ事前レンダリングし、パン中はその画像を貼るだけにする(QGIS などの
# 地図キャンバスと同じキャッシュ方式)。レイヤは「ビューポート+マージン」の範囲を
# 現在のズーム倍率で焼く。マージン内のパンは再レンダリング不要で、超えたときと
# ズーム変更時だけデバウンス付きで焼き直す。
LAYER_MARGIN_FRAC = 0.5  # ビューポート寸法に対する先読みマージン(各辺)
LAYER_MAX_PIXELS = 16_000_000  # レイヤ画像の画素数上限(超えたら解像度を落とす)
LAYER_RENDER_DELAY_MS = 120  # 連続操作(ホイールズーム等)をまとめる遅延

# Z 値: 画像とオーバーレイは 0、暗幕は 0.5、選択インスタンスは 1
Z_DIM = 0.5
Z_SELECTED = 1.0

# 点滅表示。オーバーレイの下に何があるかを、表示を切り替える操作なしで見るための
# もの。消える側は setVisible ではなく不透明度 0 で表す。setVisible(False) だと
# 当たり判定まで消え、消えている半周期のあいだインスタンスを選べなくなるため。
BLINK_INTERVAL_MS = 500  # 表示/非表示が入れ替わる間隔(1周期はこの2倍)
BLINK_OFF_OPACITY = 0.0
...
# 一筆単位の取り消し履歴の上限。履歴は「触れた矩形の直前画素」だけ持つが、
# 大きな筆で全面をなでると 1 件が画像サイズ級になるためバイト数でも抑える。
MASK_UNDO_LIMIT = 50
MASK_UNDO_MAX_BYTES = 256 * 2**20
