...
右下の「上級者設定...」ボタンで別ウィンドウが開く。

- **ブラシの使用中、右クリックで消しゴムを使う**
- **消しゴムの使用中、右クリックでブラシを使う**

ON にすると、追加モードの右ドラッグが「反対のツール」として働く(押している
あいだだけ入れ替わり、離すと元のツールへ戻る。太さはそれぞれのツールの設定値)。
塗って・消してを行き来する修正作業で、ツール切替の手数を減らすためのもの。
誤クリックが誤消去にもなり得るので既定はどちらも OFF。
...
追加モードでは `[` で選択中のブラシまたは消しゴムを2px細く、`]` で2px太くできる。
現在の割り当ては各太さスライダーの右にも表示される。
...
└── widgets/
    ...
    └── advanced_settings_dialog.py  # 上級者設定の別ウィンドウ
```
